...
void feed_forward(float *model, float *mnist, float *tmp, int num_examples, int print_pred){
  ...
  calculate_neuron<<<blocks_per_grid, threads_per_block>>>(num_examples, tmp, 128, tmp + data_offset * sizeof(float), 64, model + model_offset * sizeof(float), model + (model_offset + 128 * 64) * sizeof(float), 1);
  model_offset += 128 * 64 + 64; // weights and the bias
     
  // last layer
  blocks_per_grid = (10 * num_examples + threads_per_block - 1) / threads_per_block;
  calculate_neuron<<<blocks_per_grid, threads_per_block>>>(num_examples, tmp + data_offset * sizeof(float), 64, tmp + (data_offset + 64 * num_examples) * sizeof(float), 10, model + model_offset * sizeof(float), model + (model_offset + 64 * 10) * sizeof(float), 0);
